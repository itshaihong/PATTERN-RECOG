...
class DiscreteD:
    ...
    def rand(self, nData):
        ...
        return np.random.choice(a=range(len(self.probMass)), size=nData, p=self.probMass)
    ...
    def init(self, x):
        ...
        #very confusing, missed the max number possible,
        #maybe fObs=np.zeros(maxObs+1)?
        #still keep the first __init__?
